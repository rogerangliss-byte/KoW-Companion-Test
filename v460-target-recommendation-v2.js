(function(){
  ...
  function applyMode(){if(txt('v460TargetMode')==='max'){byId('v460TargetLevel').value=70;byId('v460TargetStars').value=5;byId('v460TargetTraining').value=180;}}
  function calculate(e){if(e){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();}var r=byId('v460TargetResult'),o=selectedOfficer();if(!r)return false;if(!o){r.innerHTML='<strong>Select an Officer first.</strong>';return false;}
    var tl=val('v460TargetLevel',70),ts=val('v460TargetStars',5),tt=val('v460TargetTraining',180),cl=val('currentLevel',0),cs=val('currentStar',0),ct=val('trainingExact',val('training',0));tl=Math.max(cl,Math.min(70,tl));ts=Math.max(cs,Math.min(5,ts));tt=Math.max(ct,Math.min(180,tt));
    ...
    var rarity=o.rarity||'Legendary',isMax=txt('v460TargetMode')==='max',readiness=String(o.season).toLowerCase()!=='original'&&o.orv>0;
