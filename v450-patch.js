...
(function(){
  ...
  function writeUI(patch){
    try{localStorage.setItem(UI_KEY,JSON.stringify(Object.assign(readUI(),patch)))}catch{}
  }
  function saveOfficerSelection(){
    const s=document.getElementById('officerSelect');
    if(!s||!s.options.length)return;
    const opt=s.options[s.selectedIndex];
    writeUI({officerValue:s.value,officerText:opt?opt.textContent:''});
  }
  function restoreOfficerSelection(){
    ...
    if(state.officerText){
      ...
      for(let i=0;i<s.options.length;i++){
        if(String(s.options[i].textContent).trim()===wanted){s.selectedIndex=i;found=true;break;}
      }
    }
    if(!found&&state.officerValue!==undefined){
      for(let i=0;i<s.options.length;i++)if(s.options[i].value===String(state.officerValue)){s.selectedIndex=i;found=true;break;}
    }
    if(found)s.dispatchEvent(new Event('change',{bubbles:true}));
  }
  function saveView(view){if(view)writeUI({view:view})}
  function restoreView(){
    const view=readUI().view;
    if(!view)return;
  ...
  document.addEventListener('click',function(e){
    const nav=e.target.closest?.('.bottom-nav button[data-view]');
    if(nav)saveView(nav.dataset.view);
    const label=e.target.closest?.('#restoreAppDataLabel');
    if(label){const input=document.getElementById('restoreAppData');if(input)input.value='';}
  },true);
  ...
  document.addEventListener('input',function(e){if(e.target&&FIELD_IDS.includes(e.target.id))saveField(e.target)},true);

  /* Restore once after the native app has completed its own initial population.
     No observer, polling loop, DOM rewriting or repeated rendering is used. */
